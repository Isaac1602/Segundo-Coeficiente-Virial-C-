...
long double  stockmayer (long double dominio, long double epsilon){
    long double r, potencial;
    r = dominio;
    potencial = 4*epsilon*(pow(1/r, 12)- pow(1/r,6));
    return potencial;
}

void integral_trapecio (long double limite_inferior, long double limite_superior, 
                         long double *rango,  long double intervalo, int nsteps, long double  T, long double epsilon,
    ...
    //snprintf(filename, sizeof(filename), "segunodcoef_sig_10eps%.2LeT%Le.dat", epsilon, T);
    //file = fopen(filename, "w");
    //fprintf(file, "Los datos del sgundo virial con sigma = 0.0 y epsilon = %Le con T = %Le son \n", epsilon, T );

    suma = rango[0]*rango[0]*(exp(-stockmayer(rango[0], epsilon)/T)-1);
    //fprintf(file, "%Le %Le\n", rango[0], suma);

    for (i = 1; i <= nsteps-2;i++){
        evalucacion_SM = rango[i]*rango[i]*(exp(-stockmayer(rango[i], epsilon)/T)-1);
        //fprintf(file, "%Le %Le\n", rango[i], evalucacion_SM);
        suma = suma + 2*evalucacion_SM;
    }
    evalucacion_SMf = rango[nsteps-1]*rango[nsteps-1]*(exp(-stockmayer(rango[nsteps-1], epsilon)/T)-1);
...
int main (){
    ...
    long double  epsilons[3] = {0.5, 1.0, 1.5};
    long double  integral;
    ...
    for (j = 0; j<= 2; j++)
    {
        snprintf(nombrearchivo, sizeof(nombrearchivo), "SegCoefVirSigma1.0Eps%.2Le.dat", epsilons[j]);
        archivo_temperatura = fopen(nombrearchivo, "w");
        printf ("entra\n ");
        for (k = 0; k<=9; k++){
            integral_trapecio(limite_inferior, limite_superior, rango, 
                            intervalo, nsteps, temperaturas[k], epsilons[j], &integral);
            
            fprintf(archivo_temperatura,"%14.8Le %14.8Le\n", temperaturas[k], integral);
            
            printf("%Le %Le \n", temperaturas[k], epsilons[j]);
            }
