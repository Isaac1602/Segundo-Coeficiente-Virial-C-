...
for k in miua:
    archivo = open("SMmiu"+str(int(k*10))+".dat","w")
    archivo.write("@    title \"Stock-Myers: Caso 1\" \n")
    archivo.write("@    xaxis  label \"r*\" \n")
    archivo.write("@    yaxis  label \"U*(r*)\" \n")

    for i in range(n):
        x = liminf+i*inter
        y = stockmyers(sigred,epsred,k,x)
        archivo.write(str(x) + "	" + str(y) + "\n")

    print(" Archivo SMmiu"+str(int(k*10))+".dat Generado")
    archivo.close()
